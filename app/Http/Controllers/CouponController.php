...
class CouponController extends Controller
{
    ...
    public function manage_coupon_process(Request $request) {
        ...
        $model->code = $request->post('value');
        $model->value = $request->post('code');
        $model->status = '1';
        $model->save();
    ...
    public function delete(Request $request, $id) {
        ...
        $model->delete();
        $request->session()->flash("message", "Coupon Delete Success");
        return redirect('admin/coupon');
    }
    public function status(Request $request, $type ,$id) {
        ...
        $model->status = $type;
        $model->save();
        $request->session()->flash("message", "Coupon Status Success");
        return redirect('admin/coupon');
    }
}
